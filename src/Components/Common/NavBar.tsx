...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchDataAndSetNotiNumber, 3000);
  ...
  async function getReqListByBuyerId(buyer_id: string) {
    ...
    try {
      ...
      var filteredList1 = myList.filter((item) => item.Complete === false);
      setNotiNumber(filteredList1.length);
          ...
          {isLoggedInUser ? (
            ...
            <>
              ...
              <IconButton style={iconStyle} onClick={handlenotiClick} onMouseLeave={handleNonotiClick} >
                ...
                <Typography><Link to="/" style={loginButtonStyle}>Logout</Link></Typography>
                </IconButton>
